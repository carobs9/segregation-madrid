...
if cfg.type_of_study != 'morans':
    sys.exit("Error: Set the type of study to 'morans'. Script stopped.")
...
# results as lists of tuples
mi_results = [ # get p values and z scores
...
global_moran = global_moran.round(3) # rounding to 3 decimal points
...
local_mi = [ # calculating the local morans i
    Moran_Local(gdf[variable], w) for variable in cfg.INCOME_VARS_OF_INTEREST
]

mi_results_local = [ # obtaining p values and z scores
...
local_moran = local_moran.round(3) # rounding to 3 decimal points
...
for var in cfg.INCOME_VARS_OF_INTEREST:
    ...
    # add the global statistics as text on the plot
